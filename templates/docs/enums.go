...
<style>
  ...
  /* Hidden radios live inside <main>, so they precede #enum_list and      */
  /* #enum_values and can drive them via the ~ sibling combinator.         */
  .enum-panel { display: none; }
  .enum-item  { border-left: 2px solid transparent; }

  {{range .Enums}}
  /* {{.Name}}: show its value panel and highlight its list item */
  #r-{{.TsName}}:checked ~ #enum_values #v-{{.TsName}} { display: block; }
  #r-{{.TsName}}:checked ~ #enum_list  label[for="r-{{.TsName}}"] {
...
{{template "HEADER_TMPL" .Name}}
    <main class="flex-1 overflow-y-auto grid grid-cols-1 xl:grid-cols-2 divide-y xl:divide-y-0 xl:divide-x divide-slate-800">
		...
		<!-- Hidden radio group: one per enum, first is the default selection -->
		{{range $i, $e := .Enums}}
        <input type="radio" name="enum" id="r-{{$e.TsName}}" class="hidden" {{if eq $i 0}}checked{{end}}>
		{{end}}

		<!-- Left Column: Enums List (labels drive the radios) -->
