...
const HEADER_TMPL = `
    <header class="bg-slate-950 border-b border-slate-800 p-4 flex items-center justify-between">
        ...
        <div class="flex" style="gap: 2rem">
            <a href="index.html" class="text-gray-300 hover:text-gray-50">Services</a>
            <a href="types.html" class="text-gray-300 hover:text-gray-50">Data Types</a>
            <a href="enums.html" class="text-gray-300 hover:text-gray-50">Enums</a>
        </div>
    </header>
`
...
const INDEX_TMPL = `
<!DOCTYPE html>
<html lang="en" class="h-full bg-slate-900 text-gray-100">
{{template "HEAD_TMPL"}}
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-6">
        <div>
			...
			<ul class="space-y-1 text-sm">
                <li>
					<a href="#authentication" class="block py-1 px-1.5 rounded text-gray-300 font-medium hover:text-white">Authentication</a>
				</li>
                <li>
					<a href="#errors" class="block py-1 px-1.5 rounded text-gray-300 font-medium hover:text-white">Errors</a>
				</li>
            </ul>
			{{range .ServiceGroups}}
				<br>
				...
				<ul class="space-y-1 text-sm">
				{{range .List}}
                    <a href="service_{{.TsName}}.html" class="block py-1 px-1.5 rounded text-gray-300 font-medium hover:text-white">{{.Name}}</a>
				{{end}}
				</ul>
			{{end}}
        </div>
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div>
			...
			<ul class="text-sm font-mono">
				{{range .List}}
                <li>
                    <a href="service_{{.TsName}}.html" class="flex items-center py-1 text-slate-300 hover:text-white">
                ...
                <pre class="bg-slate-900 border border-slate-800 rounded-lg p-4 font-mono text-xs text-emerald-400 overflow-x-auto">
{
  ...
  "error": "detailed error description"
			...
			{{range .ServiceGroups}}
			{{range .List}}
            <section id="{{.TsName}}" class="scroll-mt-16">
                <h2 class="text-2xl font-bold text-white mb-2">{{.Name}}</h2>
				{{if .Path}}<div class="font-mono text-xs text-emerald-400 mb-3">{{.Path}}</div>{{end}}
				{{range .Docs}}<p class="text-slate-300 leading-relaxed mb-4">{{.}}</p>{{end}}
            </section>
			{{end}}
			{{end}}
